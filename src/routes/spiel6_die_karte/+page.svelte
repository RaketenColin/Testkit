...
function handleKeydown(event) {
    if (event.key === 'ArrowLeft' && $spielPunkteTeam1 < 6) {
      $spielPunkteTeam1 += 1;
      audioElement.play();
      event.preventDefault();
    } else if (event.key === 'ArrowRight' && $spielPunkteTeam2 < 6) {
      $spielPunkteTeam2 += 1;
      audioElement.play();
      event.preventDefault();
    }
  ...
  function speichernUndZurueck() {
    zusatzTeam1 = $spielPunkteTeam1*20;
    zusatzTeam2 = $spielPunkteTeam2*20;
...
<canvas class="partikel-canvas" bind:this={canvas}></canvas>
  <div class="punkte-leiste">
    ...
    <span class="brett-info">Die Karte</span>
  ...
  <div class="titel-wrapper">
    <div class="titel-pille">Die Karte</div>
  </div>
