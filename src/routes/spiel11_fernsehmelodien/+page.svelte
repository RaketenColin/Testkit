<script>
  ...
  let zustand = 0;
  let fragenIndex = 0;
  let serie = fragen.Alle_Spiele.Fernsehmelodien;
  let audioElement;
  let videoElement;
  let schongedrueckt = [];
  let canvas;
  ...
  function enterDruck() {
  if (fragenIndex >= serie.length - 1 && zustand === 2) {
    return;
  }
  if (zustand === 0) {
    zustand = 1;
    ...
    zustand = 1;
    fragenIndex = Math.min(fragenIndex + 1, serie.length - 1);
...
function handleBuzzerEnter(event) {
  ...
  if (zustand !== 2 || fragenIndex >= serie.length - 1) return;
  if ($namen1.includes(player)) {
    audioElementbing.play();
    spielPunkteTeam1.update(p => p + 2);
  } else if ($namen2.includes(player)) {
    audioElementbing.play();
    spielPunkteTeam2.update(p => p + 2);
  }
}
</script>
...
<div class="full-screen-bg">
  ...
  <div class="punkte-leiste">
    ...
    <span class="brett-info">Fernsehmelodien</span>
  ...
  <div class="quiz-container">
      {#if zustand === 1}
        <audio bind:this={audioElement} src={serie[fragenIndex].Fernsehserie.Audiopfad} controls autoplay> </audio>
      {/if}
 
      {#if zustand === 2}
        <!-- svelte-ignore a11y_media_has_caption -->
        <video bind:this={videoElement} src={serie[fragenIndex].Fernsehserie.VideoPfad} controls autoplay> </video>
        <p style="font-size: 2rem; color: white; font-weight: bold; margin-top: -10px;">
        {serie[fragenIndex].Fernsehserie.Serienname}</p>
      {/if}
    </div>
</div>

<div class="titel-wrapper">
  <div class="titel-pille">Fernsehmelodien</div>
</div>
