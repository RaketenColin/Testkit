<script>
  ...
  let zustand = 0;
  let fragenIndex = 0;
  let lieder = fragen.Alle_Spiele.Songquiz;
  let audioElement;
  let schongedrueckt = [];
  ...
  function enterDruck() {
  if (fragenIndex >= lieder.length - 1 && zustand === 2) {
    return;
  }
  if (zustand === 0) {
    zustand = 1;
    ...
    zustand = 1;
    fragenIndex = Math.min(fragenIndex + 1, lieder.length - 1);
...
function handleBuzzerEnter(event) {
  ...
  if (zustand !== 2 || fragenIndex >= lieder.length - 1) return;
  const aktuellerSong = lieder[fragenIndex].Song.Songtitel;
  if ($namen1.includes(player)) {
    audioElementbing.play();
    spielPunkteTeam1.update(p => p + 4);
    $errateneSongsTeam1 = [...$errateneSongsTeam1, aktuellerSong];
  } else if ($namen2.includes(player)) {
    audioElementbing.play();
    spielPunkteTeam2.update(p => p + 4);
    $errateneSongsTeam2 = [...$errateneSongsTeam2, aktuellerSong];
  }
}
</script>
...
<div class="full-screen-bg">
  ...
  <div class="punkte-leiste">
    ...
    <span class="brett-info">Songquiz</span>
  ...
  <div class="quiz-container">
      {#if zustand === 1}
        <audio bind:this={audioElement} src={lieder[fragenIndex].Song.Songpfad} controls autoplay> </audio>
      {/if}
 
      {#if zustand === 2}
        <img src={lieder[fragenIndex].Song.Bildpfad} alt="Bild zur Songfrage" />
      {/if}
    </div>
</div>

<div class="titel-wrapper">
  <div class="titel-pille">Songquiz</div>
</div>
