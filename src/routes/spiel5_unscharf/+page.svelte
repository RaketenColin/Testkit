<script>
  ...
  let zustand = 0;
  let fragenIndex = 0;
  let unscharfePersonen = fragen.Alle_Spiele.Unscharf;
  let videoElement;
  let schongedrueckt = [];
  let canvas;
  ...
  function enterDruck() {
  if (fragenIndex >= unscharfePersonen.length - 1 && zustand === 2) {
    return;
  }
  if (zustand === 0) {
    zustand = 1;
    ...
    zustand = 1;
    fragenIndex = Math.min(fragenIndex + 1, unscharfePersonen.length - 1);
...
function handleBuzzerEnter(event) {
  ...
  if (zustand !== 2 || fragenIndex >= unscharfePersonen.length - 1) return;
  if ($namen1.includes(player)) {
    audioElementbing.play();
    spielPunkteTeam1.update(p => p + 3);
  } else if ($namen2.includes(player)) {
    audioElementbing.play();
    spielPunkteTeam2.update(p => p + 3);
  }
}
</script>
...
<div class="full-screen-bg">
  ...
  <div class="punkte-leiste">
    ...
    <span class="brett-info">Unscharf</span>
    ...
    <div class="quiz-container">
      {#if zustand === 1}
        <video bind:this={videoElement} src={unscharfePersonen[fragenIndex].Person.Videopfad} controls autoplay muted> </video>
      {/if}
 
      {#if zustand === 2}
        <img src={unscharfePersonen[fragenIndex].Person.Bildpfad} alt="Bild" />
        <p style="font-size: 2rem; color: white; font-weight: bold; margin-top: -10px;">
        {unscharfePersonen[fragenIndex].Person.Name}</p>
      {/if}
      </div>
</div>

<div class="titel-wrapper">
  <div class="titel-pille">Unscharf</div>
</div>
