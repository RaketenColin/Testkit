<script>
  ...
  let zustand = 0; 
  let fragenIndex = 0;
  let buchfragen = fragen.Alle_Spiele.Buchcover;
  let canvas;
...
function enterDruck() {
  if (fragenIndex >= buchfragen.length - 1 && zustand === 2) {
    return;
  }
  if (zustand === 0) {
    zustand = 1;
    ...
    zustand = 1;
    fragenIndex = Math.min(fragenIndex + 1, buchfragen.length - 1);
...
function handleBuzzerEnter(event) {
  ...
  if (zustand !== 2 || fragenIndex >= buchfragen.length - 1 ) return;
...
<canvas class="partikel-canvas" bind:this={canvas}></canvas>
  <div class="punkte-leiste">
    ...
    <span class="brett-info">Buchcover</span>
    ...
    <div class="quiz-container">
      {#if zustand === 1}
        <img class="buchcover-bild" src={buchfragen[fragenIndex].Buchtitel.Fragebuchpfad} alt="Frage" />
      {/if}
  
      {#if zustand === 2}
          <img class="buchcover-bild" src={buchfragen[fragenIndex].Buchtitel.Antwortbuchpfad} alt="Antwort" />
      {/if}
    </div>
</div>
...
<div class="titel-wrapper">
  <div class="titel-pille">Buchcover</div>
</div>
