<script>
  ...
  let zustand = 0; 
  let fragenIndex = 0;
  let tierarten = fragen.Alle_Spiele.Tiere_schmecken;
  let canvas;
  let audioElement;
  ...
  function enterDruck() {
  if (fragenIndex >= tierarten.length - 1 && zustand === 2) {
    return;
  }
  if (zustand === 0) {
    zustand = 1;
    ...
    zustand = 1;
    fragenIndex = Math.min(fragenIndex + 1, tierarten.length - 1);
  }
}
...
function handleKeydown(event) {
    ...
    if (event.key === 'ArrowLeft'&& $spielPunkteTeam1 < 8) {
      $spielPunkteTeam1 += 1;
      audioElement.play();
      event.preventDefault();
    } else if (event.key === 'ArrowRight' && $spielPunkteTeam2 < 8) {
      $spielPunkteTeam2 += 1;
      audioElement.play();
      event.preventDefault();
    }
...
function speichernUndZurueck() {
  zusatzTeam1 = $spielPunkteTeam1*10;
  zusatzTeam2 = $spielPunkteTeam2*10;
...
<canvas class="partikel-canvas" bind:this={canvas}></canvas>
  <div class="punkte-leiste">
    ...
    <span class="brett-info">Tiere schmecken</span>
  ...
  <div class="titel-wrapper">
    <div class="titel-pille">Tiere schmecken</div>
  </div>
...
<audio bind:this={audioElement} src="/General/ding.mp3" controls> </audio>
  ...
  <div class="quiz-container">
      {#if zustand === 1}
        <img class="tiereschmecken-bild" src={tierarten[fragenIndex].Hintergrundpfad} alt="Frage" />
      {/if}
  
      {#if zustand === 2}
          <img class="tiereschmecken-bild" src={tierarten[fragenIndex].Bildpfad} alt="Antwort" />
      {/if}
  </div>
</div>
