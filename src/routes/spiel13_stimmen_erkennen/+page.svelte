<script>
  ...
  let zustand = 0;
  let fragenIndex = 0;
  let promi = fragen.Alle_Spiele.Stimmen_erkennen;
  let audioElement;
  let videoElement;
  let schongedrueckt = [];
  let canvas;
  ...
  function enterDruck() {
  if (fragenIndex >= promi.length - 1 && zustand === 2) {
    return;
  }
  if (zustand === 0) {
    zustand = 1;
    ...
    zustand = 1;
    fragenIndex = Math.min(fragenIndex + 1, promi.length - 1);
...
function handleBuzzerEnter(event) {
  ...
  if (zustand !== 2 || fragenIndex >= promi.length - 1) return;
...
<div class="full-screen-bg">
  ...
  <div class="punkte-leiste">
    ...
    <span class="brett-info">Stimmen erkennen</span>
  ...
  <div class="quiz-container">
      {#if zustand === 1}
        <audio bind:this={audioElement} src={promi[fragenIndex].Person.Audiopfad} controls autoplay> </audio>
      {/if}
 
      {#if zustand === 2}
        <!-- svelte-ignore a11y_media_has_caption -->
        <video bind:this={videoElement} src={promi[fragenIndex].Person.Videopfad} controls autoplay> </video>
        <p style="font-size: 2rem; color: white; font-weight: bold; margin-top: -10px;">
        {promi[fragenIndex].Person.Name}</p>
      {/if}
    </div>
</div>

<div class="titel-wrapper">
  <div class="titel-pille">Stimmen erkennen</div>
</div>
