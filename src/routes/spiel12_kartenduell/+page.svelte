...
function handleKeydown(event) {
    if (event.key === 'ArrowLeft' && $spielPunkteTeam1 < 22) {
      $spielPunkteTeam1 += 1;
      audioElement.play();
      event.preventDefault();
    } else if (event.key === 'ArrowRight' && $spielPunkteTeam2 < 22) {
      $spielPunkteTeam2 += 1;
      audioElement.play();
      event.preventDefault();
    }
...
<canvas class="partikel-canvas" bind:this={canvas}></canvas>
  <div class="punkte-leiste">
    ...
    <span class="brett-info">Kartenduell</span>
  ...
  <div class="titel-wrapper">
    <div class="titel-pille">Kartenduell</div>
  </div>
