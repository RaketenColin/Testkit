<script>
  ...
  let zustand = 0; 
  let fragenIndex = 0;
  let spielbretter = fragen.Alle_Spiele.Brettspiele;
  let canvas;
...
function enterDruck() {
  if (fragenIndex >= spielbretter.length - 1 && zustand === 2) {
    return;
  }
  if (zustand === 0) {
    zustand = 1;
    ...
    zustand = 1;
    fragenIndex = Math.min(fragenIndex + 1, spielbretter.length - 1);
...
function handleBuzzerEnter(event) {
  ...
  if (zustand !== 2 || fragenIndex >= spielbretter.length - 1) return;
  if (player) {
    if ($namen1.includes(player)) {
      audioElementbing.play();
      spielPunkteTeam1.update(p => p + 5);
    } else if ($namen2.includes(player)) {
      audioElementbing.play();
      spielPunkteTeam2.update(p => p + 5);
    }
  }
}
</script>
...
<canvas class="partikel-canvas" bind:this={canvas}></canvas>
  <div class="punkte-leiste">
    ...
    <span class="brett-info">Spiele</span>
    ...
    <div class="quiz-container">
      {#if zustand === 1}
        <img class="spiele-bild" src={spielbretter[fragenIndex].Spielbrett.Bildpfad} alt="Frage" />
      {/if}
  
      {#if zustand === 2}
          <img class="spiele-bild" src={spielbretter[fragenIndex].Spielbrett.Bildpfad} alt="Frage" />
          <p style="font-size: 2rem; color: white; font-weight: bold; margin-top: 700px;">
          {spielbretter[fragenIndex].Spielbrett.Spielname}</p>
      {/if}
    </div>
</div>

<div class="titel-wrapper">
    <div class="titel-pille">Spiele</div>
  </div>
